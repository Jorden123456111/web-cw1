...
router.put('/:id', authenticate, validate([
  param('id').isInt().withMessage('Match ID must be an integer.'),
  body('homeTeamId').optional().isInt().withMessage('homeTeamId must be an integer.'),
  body('awayTeamId').optional().isInt().withMessage('awayTeamId must be an integer.'),
  body('homeScore').optional().isInt({ min: 0 }).withMessage('homeScore must be a non-negative integer.'),
  body('awayScore').optional().isInt({ min: 0 }).withMessage('awayScore must be a non-negative integer.'),
  body('matchDate').optional().isDate().withMessage('matchDate must be a valid date (YYYY-MM-DD).'),
  body('season').optional().trim().notEmpty().withMessage('Season cannot be empty.'),
  body('competition').optional().trim().notEmpty().withMessage('Competition cannot be empty.'),
  body('status').optional().isIn(['scheduled', 'completed', 'cancelled', 'postponed']).withMessage('Invalid status value.'),
]), updateMatch);
