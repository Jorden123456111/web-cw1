# Football Statistics API — Documentation
...
The API uses **JSON Web Tokens (JWT)** for authentication. Write operations (POST, PUT, DELETE) require a valid token; read operations (GET) are public.
...
| search | string | — | Search by team name (partial match) |
...
#### DELETE `/api/teams/:id` *(Auth required)*
...
#### DELETE `/api/players/:id` *(Auth required)*
...
| teamId | integer | — | Matches involving this team (home or away) |
